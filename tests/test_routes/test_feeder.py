...
def test_feeder_update_device(client: TestClient, with_stored_recipe: None, mocker):
    ...
    timezone = mocker.patch("feeder.api.routers.feeder.router.client.send_cmd_utc_offset")
    front_button = mocker.patch("feeder.api.routers.feeder.router.client.send_cmd_button")
...
def test_feeder_update_device_seperate_calls(client: TestClient, with_stored_recipe: None, mocker):
    from tests.test_database_models import SAMPLE_DEVICE_HID, SAMPLE_GATEWAY_HID
...
def test_feeder_reboot(client: TestClient, with_registered_device: None, mocker):
    ...
    response = client.post(
        f"/api/v1/feeder/{SAMPLE_DEVICE_HID}/restart"
    )
    assert response.status_code == 200
...
def test_feeder_feed(client: TestClient, with_registered_device: None, mocker):
    ...
    response = client.post(
        f"/api/v1/feeder/{SAMPLE_DEVICE_HID}/feed",
        json={"portion": 0.0625}
    )
    assert response.status_code == 200
    cmd.assert_called_once_with(
...
def test_feeder_raw(client: TestClient, with_registered_device: None, mocker):
    ...
    response = client.post(
        f"/api/v1/feeder/{SAMPLE_DEVICE_HID}/raw",
        json={
            "command": "test",
            "args": {}
        }
    ...
    assert response.json()["detail"] == "Raw communication only available in DEBUG mode!"
    ...
    assert response.status_code == 200
    cmd.assert_called_once_with(
        SAMPLE_GATEWAY_HID, SAMPLE_DEVICE_HID, "test", {}
    )
...
def test_feeder_get_recipe_none(client: TestClient, with_registered_device: None):
    ...
    response = client.get(
        f"/api/v1/feeder/{SAMPLE_DEVICE_HID}/recipe"
    )
    assert response.status_code == 400
