...
class EditFeederModalContainer extends React.Component {
    ...
    handleSetManualFeedPortion(portion, commit = false) {
        ...
        this.setState({manualFeedPortion: portion}, () => {
            if (commit) {
                this.props.dispatchSetRecipe(this.state.feeder.hid, portion * tbspInCup).then(() => {
    ...
    render() {
        ...
        return (
            <EditFeederModalComponent
                ...
                toggleConfirmDelete={(show) => this.setState({showConfirmDelete: show})}
                ...
                handleRegisterHopperLevelControl={(setHopperLevelValue) => this.hopperLevelValue = setHopperLevelValue}
            />
        )
    }
}
...
const EditFeederModal = withRouter(connect(
    (state) => {
        const {
