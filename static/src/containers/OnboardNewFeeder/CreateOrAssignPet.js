    ...
    render() {
        const petArray = this.state.pets.map(
            (pet) => {
                ...
                return <PetAvatar name={pet.name} size={100} image={pet.image} showName
            ...
            <>
                <Modal.Body style={{textAlign: "center"}} className={"py-4"}>
                    ...
                    {this.state.creatingNewPet ?
                        ...
                        <>
                            {petArray}
                            <PetAvatar name={"New"} size={100} image={AddNewIcon} showName
                ...
                <Modal.Footer>
                    {this.state.creatingNewPet ?
                        <Button variant={"success"} onClick={() => this.submitFormAction()}>Save</Button> :
                        <Button variant={"success"} onClick={this.handleAssignAndProgress}>Assign</Button>
                    }
...
const CreateOrAssignPet = withRouter(connect(
    (state) => {
        ...
        return {
            ...
            dispatchCreatePet(name, animal_type, weight, birthday, activity_level, image, device_hid) {
                return dispatch(createPetAction(name, animal_type, weight, birthday, activity_level, image, device_hid))
            },
            dispatchModifyPet(pet_id, name = null, animal_type = null, weight = null, birthday = null, activity_level = null, image = null, device_hid = null) {
                return dispatch(modifyPetAction(pet_id, name, animal_type, weight, birthday, activity_level, image, device_hid))
            }
        };
    }
