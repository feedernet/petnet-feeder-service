...
export class CreateRecipeContainer extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.handleDispenseAndWeighLoop = this.handleDispenseAndWeighLoop.bind(this)
    ...
    async handleDispenseAndPoll() {
        ...
        if (!this.state.manualEntry) {
            ...
            while ((lastTimeAfter === null || lastTimeAfter <= lastTimeBefore) && new Date() < timeoutTime && !this.state.manualEntry) {
            ...
            const portionWeight = this.props.getFeedHistoryState.history[0].grams_actual;
    ...
    handleCreateRecipe() {
        if (this.state.measuredDensity !== null && this.state.measuredDensity > 0) {
            this.props.dispatchSetRecipe(this.props.deviceHid, this.state.measuredDensity).then(() => {
            ...
            <>
                <Modal.Body style={{textAlign: "center"}} className={"pt-4"}>
                    {this.state.errorWeighing ?
                        <Alert variant={"danger"}>The automatic measuring process didn't work!
                            Please manually enter food weight.</Alert> : null
                    }
                    <h2>What does their food weigh?</h2>
                    <p className={"text-muted mt-1"}>
                        To make sure we are dispensing the right amount of food, we need to know how dense it
                        is.
                    </p>
                    ...
                    {!this.state.manualEntry ?
                        <>
                            <p className={"text-muted mt-1"}>
                                We are going to automatically dispense three 1 tbsp portions to get the weight
                                of their food.
                            </p>
                        ...
                        <>
                            <p className={"mt-1"}>
                                Please weigh out a single tablespoon of food and enter the weight below.
                            </p>
                            <InputGroup className="mb-3">
                                <FormControl
                                    ...
                                    onChange={(event) => this.setState({measuredDensity: Math.round(event.target.value)})}
                                />
                                <InputGroup.Append>
                    ...
                    {!this.state.manualEntry ?
                        <>
                            <FoodWeightBubble loading={this.state.isCurrentlyMeasuring[0]}
                                              weight={this.state.measuredWeights[0]}/>
                            <FoodWeightBubble loading={this.state.isCurrentlyMeasuring[1]}
                                              weight={this.state.measuredWeights[1]}/>
                            <FoodWeightBubble loading={this.state.isCurrentlyMeasuring[2]}
                                              weight={this.state.measuredWeights[2]}/>
                        </> : null}

                    {!this.state.manualEntry ?
                        <p className={"mt-3"}>Average density: {this.state.measuredDensity} g/tbsp</p> : null}


                </Modal.Body>
                <Modal.Footer>
                    {!this.state.manualEntry ?
                        <Button variant={"secondary"} onClick={() => this.setState({manualEntry: true})}>
                            Manual Entry
                        </Button> : null}

                    {!this.state.manualEntry && this.state.measuredWeights[2] === null ?
                        <Button variant={"success"} onClick={() => this.handleDispenseAndWeighLoop()}
                    ...
                    {this.state.manualEntry ?
                        <>
                            ...
                            <Button variant={"success"} onClick={() => this.handleCreateRecipe()}>
                                Submit
                            </Button>
                        </> : null}
                </Modal.Footer>
...
const CreateRecipe = withRouter(connect(
    (state) => {
        ...
        return {
            ...
            dispatchSetRecipe(deviceId, g_per_tbsp = null) {
                return dispatch(setRecipeAction(deviceId, g_per_tbsp, 1, "Initial Recipe", 1))
            }
        };
    }
