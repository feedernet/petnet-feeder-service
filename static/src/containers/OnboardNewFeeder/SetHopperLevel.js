...
export class SetHopperLevelContainer extends React.Component {
    ...
    handleSubmitForm(values, actions) {
        this.props.dispatchSetHopperLevel(this.props.deviceHid, values.level).then(() => {
        ...
        return (
            <>
                <Modal.Body style={{textAlign: "center"}} className={"py-4"}>
                    ...
                    <div className={"mx-4"}>
                        <HopperLevelFormComponent handleFormSubmit={this.handleSubmitForm}
                                                  handleRegisterFormSubmit={this.handleRegisterFormSubmit}/>
                    </div>
                </Modal.Body>
                <Modal.Footer>
                    <Button variant={"success"} onClick={() => this.submitFormAction()}>Next</Button>
                </Modal.Footer>
            </>
        )
    }
...
const SetHopperLevel = withRouter(connect(
    (state) => {
