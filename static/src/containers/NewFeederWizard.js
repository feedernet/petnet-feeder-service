...
class NewFeederWizardContainer extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (!prevProps.newFeederWizardState.show && this.props.newFeederWizardState.show) {
            if (this.wizard !== null) {
    ...
    render() {
        ...
        return (
            <Modal show={showState} onHide={this.props.dispatchDismissFeederWizard} centered>
                <Modal.Header closeButton>
                    <Modal.Title className={"h5"}>New Feeder Wizard</Modal.Title>
                </Modal.Header>
                <StepWizard instance={this.registerSetupWizard} transitions={{}} isLazyMount>
...
const NewFeederWizard = withRouter(connect(
    (state) => {
