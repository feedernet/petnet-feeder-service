...
import {
    ...
    mdiCakeVariant,
    mdiWeight,
    mdiRunFast, mdiFoodApple, mdiClock, mdiPencil,
} from '@mdi/js';
            ...
            <Step transition={"scale"}>
                {({accomplished}) => {
                    ...
                    if (accomplished && dispensed && !event.result.fail) {
                        ...
                        tooltipMessage = `Error Dispensing: ${event.result.error ? event.result.error : "Unknown Error"}`
                        indicator = <img src={MealError} alt={"scheduled meal"} height={24} width={24}
                                         style={{filter: 'drop-shadow(0 2px 4px rgba(0,0,0, 0.5))'}}/>
                    } else if (accomplished && !dispensed) {
                        tooltipMessage = "Feeder has yet to dispense the meal."
                        indicator = <img src={MealMissed} alt={"scheduled meal"} height={24} width={26}
                                         style={{filter: 'drop-shadow(0 2px 4px rgba(0,0,0, 0.5))'}}/>
                    }

                    return <OverlayTrigger
                        placement="top"
                        overlay={<Tooltip id={`result-tooltip-${event.time}`}>{tooltipMessage}</Tooltip>}
                    >
                ...
                <Container>
                    <Row>
                        <Col md={12} lg={3} xl={2} style={{padding: '0 10px'}}>
                            <img src={props.pet.image} alt={props.pet.name} style={{
                                borderRadius: '50%',
                                height: 150,
                                width: 150,
                                margin: '10px auto',
                            ...
                            <Row style={{display: "flex", justifyContent: "center"}}>
                                <Col sm={12} md={7} lg={8}>
                                    ...
                                    <Row className={"text-muted mt-1 px-2"}>
                                        <Col className={"py-2 pl-2 pr-1"} xs={3} style={{whiteSpace: "nowrap"}}>
                                            <Icon path={props.pet.animal_type === "cat" ? mdiCat : mdiDog}
                                                  size={.75}/> {props.pet.animal_type === "cat" ? "Cat" : "Dog"}
                                        </Col>
                                        <Col className={"py-2 pl-2 pr-2 d-none d-sm-block"} xs={3}
                                             style={{whiteSpace: "nowrap", overflow: "hidden"}}>
                                            <Icon path={mdiCakeVariant} size={.75}/> {`${petAge} years`}
                                        </Col>
                                        <Col className={"py-2 pl-2 pr-2 d-sm-none"} xs={3}
                                             style={{whiteSpace: "nowrap", overflow: "hidden"}}>
                                            <Icon path={mdiCakeVariant} size={.75}/> {`${petAge}yrs`}
                                        </Col>
                                        <Col className={"py-2 pl-2 pr-2"} xs={3}
                                             style={{whiteSpace: "nowrap", overflow: "hidden"}}>
                                            <Icon path={mdiWeight} size={.75}/> {petWeightPounds}lbs
                                        </Col>
                                        <Col className={"py-2 pl-2 pr-2"} xs={3}
                                             style={{whiteSpace: "nowrap", overflow: "hidden"}}>
                                            <Icon path={mdiRunFast} size={.75}/> {petActivity}
                                        </Col>
                                    </Row>
                                    <div className={"mt-4 mb-4"}>
                                        <ProgressBar
                                            ...
                                            text={schedulePcts.length === 0 ? "No Scheduled Meals" : null}
                                        >
                                            {scheduleSteps}
                                        </ProgressBar>
                                    </div>
