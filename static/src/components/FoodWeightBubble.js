    ...
    return (
        ...
        >
            {props.loading && props.weight === null ?
                <Spinner animation="border" role="status" variant="light" style={{marginTop: 22}}>
                    <span className="sr-only">Dispensing...</span>
                </Spinner> : null
            }
            {props.weight !== null ?
                <h2 className={"text-light mt-3"}>{props.weight}g</h2> : null
            }
            {!props.loading && !props.weight ?
                <h2 className={"text-light mt-3"}>0g</h2> : null
            }
        </div>
    );
};