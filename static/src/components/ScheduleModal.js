...
import {mdiClockOutline, mdiCup, mdiSquareEditOutline, mdiDelete} from "@mdi/js";
...
export const ScheduleModalComponent = function (props) {
    let scheduledEvents = <Card bg={"primary"} className={"text-light text-center mb-2"}>
        <Card.Body>
            No Meals Scheduled
        </Card.Body>
    </Card>
    if (props.hasOwnProperty("events") && props.events && props.events.length > 0) {
        scheduledEvents = props.events.map((event) =>
            <Card bg={event.enabled ? "primary" : "info"} className={"text-light mb-3"}>
                <Card.Body>
                    <Row>
                        <Col xs={12} sm={9}>
                            <h3>{event.name}</h3>
                            <p className={'mb-0 mr-3'} style={{display: 'inline-block'}}>
                                <Icon path={mdiCup}
                                      size={.75}/> {displaySizes[event.portion]} {event.portion > 1 ? "cups" : "cup"}
                            </p>
                            <p className={'mb-3'} style={{display: 'inline-block'}}>
                                <Icon path={mdiClockOutline} size={.75}/> {secSinceMidnightFormatter(event.time)}
                            </p>
                        </Col>
                        <Col xs={12} sm={3}>
                            <Button size="sm" variant={"light"} className={"mt-2 mb-1"} style={{width: '100%'}}
                                    onClick={() => props.startEdit(false, event)}>
                                <Icon path={mdiSquareEditOutline} size={.75}/> Edit
                            </Button>
                            <Button size="sm" variant={"danger"} className={"mt-1 mb-0"}
                                    style={{width: '100%'}} onClick={() => props.handleDeleteEvent(event.event_id)}>
    ...
    if (props.editMode) {
        body = <Formik
            initialValues={Object.keys(props.targetEvent).length !== 0 ? props.targetEvent : {
                name: "",
                time: 39600,
                portion: 1,
                enabled: true
                                ...
                                <Col xs={12} md={7}>
                                    ...
                                    <Form.Group>
                                        ...
                                        <h1 style={{textAlign: "center"}}>
                                            {displaySizes[values.portion]} {values.portion > 1 ? "cups" : "cup"}
                                        </h1>
                                        <div className={'mx-2'} style={{marginTop: 0, marginBottom: 40}}>
                                            <FoodVolumeSlider value={values.portion}
                                                              onChange={(amount) => setFieldValue("portion", amount)}/>
                                        </div>
                                    ...
                                    <Form.Group>
                                        ...
                                        <ButtonGroup aria-label="Basic example" style={{width: "100%"}}>
                                            <Button variant={values.enabled ? "success" : "light"}
                                                    onClick={() => setFieldValue("enabled", true)}
                                                    disabled={props.newEvent}
                                            >
                                                On
                                            </Button>
                                            <Button variant={!values.enabled ? "danger" : "light"}
                                    ...
                                    <Form.Group>
                                        ...
                                        <div style={{margin: "0 auto", display: "table"}}>
                                            ...
                                            <TimeKeeper
                                                ...
                                                onChange={(newTime) => setFieldValue("time", twentyFourHourToSeconds(newTime.hour, newTime.minute))}
                                            />
                                        </div>
                                    </Form.Group>
                                </Col>
